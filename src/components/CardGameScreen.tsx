...
const CARD_PAIRS = [
  ...
  { pairId: 6, emoji: '🎪', name: '은평대전부스', colorBg: 'bg-sky-50 border-sky-300 text-sky-600' },
];
...
export const CardGameScreen: React.FC<CardGameScreenProps> = ({
  ...
  const handleVictory = async (finalMoves: number) => {
    ...
    const currentNick = playerNick.trim() || '은평청소년';
    ...
    <div className="max-w-4xl mx-auto px-4 py-6 sm:py-10 space-y-6">
      ...
      <div className="text-center space-y-2">
        ...
        <p className="text-xs sm:text-sm text-slate-600 max-w-md mx-auto">
          짝을 맞춰 카드를 가장 빠르게 뒤집어보세요! 은평대전 현장 참가자 모두의 기록이 실시간으로 집계됩니다.
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 bg-white rounded-3xl border border-slate-200 p-5 sm:p-6 shadow-sm space-y-5">
          ...
          {/* Cards Grid */}
          <div className="grid grid-cols-4 gap-2.5 sm:gap-3 aspect-4/3 min-h-[300px]">
            {cards.length === 0 ? (
              <div className="col-span-4 flex flex-col items-center justify-center p-8 bg-slate-50 rounded-2xl border border-dashed border-slate-200 text-center space-y-3">
              ...
              cards.map((card, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`relative rounded-2xl flex flex-col items-center justify-center transition-all duration-300 transform select-none cursor-pointer border-2 ${
                      showFace
                        ? `${card.colorBg} border-current shadow-sm scale-100`
                        : 'bg-slate-900 border-slate-800 hover:bg-slate-800 hover:scale-[1.02] shadow-xs'
                    }`}
                  >
                    {showFace ? (
                      <div className="flex flex-col items-center justify-center p-1">
                        <span className="text-2xl sm:text-3xl filter drop-shadow-xs">{card.emoji}</span>
                        <span className="text-[10px] sm:text-[11px] font-bold mt-1 tracking-tight truncate max-w-[65px] sm:max-w-none">
                          {card.name}
                        </span>
                      </div>
                    ) : (
