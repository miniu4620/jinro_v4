...
export const ResultCard: React.FC<ResultCardProps> = ({
  ...
  // Fetch Gemini personalized quote
  useEffect(() => {
    let isMounted = true;
      ...
      .then((resData) => {
        if (isMounted) {
          if (resData?.description) {
            ...
            setAiQuote(`${nickname || '참가자'}님은 ${data.nameKo}!`);
          }
  ...
  }, [nickname, personalityType, data.nameKo]);

  const handleCopyText = async () => {
    const textToCopy = `[MINIU WORKSHOP] ${nickname || '은평이'}님의 성향 진단 결과는 ${data.emoji} ${data.nameKo}!\n\n✨ 성향 키워드: ${data.keywords.join(' ')}\n🎓 추천 전공: ${data.recommendedMajors.slice(0, 3).join(', ')}\n\n미니유공방과 함께 나의 전공 성향을 찾아보세요!`;
    try {
    ...
    <div className="max-w-2xl mx-auto px-4 py-6 sm:py-10 space-y-6">
      ...
      >
        ...
        <div className="flex-1 flex flex-col p-6 sm:p-8 items-center text-center">
          {/* Badge: MINIU WORKSHOP */}
          <div className={`mt-2 mb-3 px-4 py-1.5 rounded-full text-xs sm:text-sm font-extrabold tracking-wider border ${theme.badge}`}>
            MINIU WORKSHOP
          </div>
          ...
          {/* Gemini Personalized Quote */}
          <div className="max-w-md mx-auto mb-8 text-center">
            <div className="inline-flex items-center gap-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-2">
            ...
            {isLoadingQuote ? (
              <div className="flex items-center justify-center gap-2 py-2 text-xs text-slate-400 font-medium">
                ...
                <span>{nickname || '은평이'}님을 위한 응원 문구를 빚고 있어요...</span>
              </div>
            ) : (
        ...
        <div className={`${theme.bannerBg} p-5 sm:p-6 flex flex-col sm:flex-row items-center justify-between text-white gap-3`}>
          <div className="text-center sm:text-left">
            <p className="text-xs opacity-80 uppercase tracking-widest font-semibold mb-1">MINIU WORKSHOP</p>
            <p className="text-base sm:text-lg font-bold">카드맞추기 랭킹 도전</p>
            <p className="text-xs text-white/90 mt-0.5">과일 카드를 빠르게 맞춰 은평대전 명예의 전당에 이름을 올려보세요!</p>
          </div>
          <button
            type="button"
